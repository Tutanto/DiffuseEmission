...
saved_models = path_to_results / 'single_model' / f'strategy_{strategy}' / f'tol_{tol}' / diffuse / f"{file_name}_ene_{e_min}_{e_max}_bin_{bin}_binsz_{binsz}" / "models"
saved_jsons = path_to_results / 'single_model' / f'strategy_{strategy}' / f'tol_{tol}' / diffuse / f"{file_name}_ene_{e_min}_{e_max}_bin_{bin}_binsz_{binsz}" / "jsons"
saved_models.mkdir(parents=True, exist_ok=True)
saved_jsons.mkdir(parents=True, exist_ok=True)

path = list(models.rglob("08*.yaml"))
...
logger = logging_conf(path_to_logs, f"single_analysis_{filename}.log")
...
if (saved_models / result_model).is_file() or (saved_jsons / result_json).is_file():
    ...
    if fit.backend == "minuit":
        ...
        if path[0].with_suffix('').name == '00_nullhypothesis':
            model_name = None
        else:
